...
public class LegacyTypedDataEncoder {
    public List<StructuredDataV1.LegacyTypedData> jsonMessageArray;

    public LegacyTypedDataEncoder() {
    }

    public LegacyTypedDataEncoder(String jsonMessageInString) throws IOException {
        this.jsonMessageArray = JsonUtils.getObjectList(jsonMessageInString, StructuredDataV1.LegacyTypedData.class);
    }



    public static byte[] legacyTypedSignatureHash(List<StructuredDataV1.LegacyTypedData> data) throws IOException {
        List<Object> _data = data.stream().map(StructuredDataV1.LegacyTypedData::getValue).collect(Collectors.toList());
        List<String> _types = data.stream().map(StructuredDataV1.LegacyTypedData::getType).collect(Collectors.toList());
        List<Object> _schemas = data.stream().map(d -> d.getType() + " " + d.getName()).collect(Collectors.toList());
        List<String> stringTypes = data.stream().map(item -> "string").collect(Collectors.toList());

        return soliditySha3(
                Arrays.asList("bytes32", "bytes32"),
                Arrays.asList(
                        soliditySha3(stringTypes, _schemas),
        ...
        if (isArray(type)) {
            ...
            if (!isArray(subType)) {
                ...
                if (!arraySize.equals("dynamic")) {
                    ...
                    if (arraySizeInt != 0 && listObj.size() > arraySizeInt) {
